  ...
  return users.value
    .filter((user) => {
      ...
      const matchesDepartment =
        lastSearchedFilters.value.department === "" ||
        user.usrDptName === lastSearchedFilters.value.department;

      const matchesDivision =
        lastSearchedFilters.value.division === "" ||
        user.usrStName === lastSearchedFilters.value.division;

      const matchesRole =
        lastSearchedFilters.value.role === "" ||
        user.usrRolName === lastSearchedFilters.value.role;

      return (
        matchesSearch && matchesDepartment && matchesDivision && matchesRole
      );
    })
...
onMounted(async () => {
  loading.value = true;
  try {
    await Promise.all([store.getAllUsers()]);
    await nextTick();
  } catch (error) {
...
<template>
  <div class="flex flex-col text-center">
    <div
      class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 mb-4 w-full"
    >
    ...
    >
      <table class="table-auto w-full text-center text-black">
        ...
        <tbody v-else>
          <tr
            v-if="!users?.length || filteredUsers.length === 0"
            v-for="n in 10"
            :key="n"
            class="h-[50px]"
          >
          ...
          >
            <template v-if="user">
              ...
              <th class="py-3 px-2 w-20 text-start font-[100]">
                {{ user.usrDptName }}
              </th>
              ...
              <th class="py-[10px] px-2 w-24 text-center">
                <span class="flex justify-center">
                  <div
                    @click="() => navigateToDetail(user.usrId.toString())"
