...
<div v-if="!isHiddenTypeVehical">
  ...
  <div v-if="!isHiddenPublic" class="flex flex-col space-y-4">
    ...
    >
      ...
      >
        <div v-for="(item, index) in vehicleRow" :key="'vehicle-' + index" class="flex w-full space-x-4">
          <p
            :class="{
...
<div v-if ="!isHiddenTypeVehical">
 <div v-if="!isHiddenPublic" class="mb-20"> </div>
  ...
  <div v-if="!isHiddenPrivate" class="flex flex-col space-y-4">
    ...
    <div v-for="(item, index) in expenseRow" :key="'expense-' + index" class="flex flex-col space-y-4">
      <div :class="{ 'text-gray-400': item.isDisabled }" class="flex items-center justify-between w-full">
        <div class="flex w-full space-x-4">
          <p :class="[{ 'text-gray-400': item.isDisabled, 'text-black': !item.isDisabled }, 'text-sm px-3']">
            {{ index + 1 }}
          </p>
          <div v-if="!item.isSubmitted">
            <input
              type="text"
              v-model="item.rqt_name"
              placeholder="ประเภทเบิกค่าใช้จ่าย"
              :class="['w-full px-2 py-1 border rounded', { 'text-gray-400': item.isDisabled, 'text-black': !item.isDisabled }]"
              :disabled="item.isDisabled"
            />
          </div>
          <div  v-else>
            <p :class="{ 'text-gray-400': item.isDisabled, 'text-black': !item.isDisabled }" class="text-sm">
              {{ item.dataMap }} 
            </p>
            ...
            <div class="flex items-center space-x-1">
              <template v-if="!item.isIconChanged">
                <svg class="w-[24px] h-[24px] text-gray-800 dark:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">
                  <path stroke="gray" stroke-width="1.5" d="M21 12c0 1.2-4.03 6-9 6s-9-4.8-9-6c0-1.2 4.03-6 9-6s9 4.8 9 6Z"/>
                  <path stroke="gray" stroke-width="2" d="M15 12a3 3 0 1 1-6 0 3 3 0 0 1 6 0Z"/>
                </svg>
              </template>
              <template v-else>
                <svg class="w-[24px] h-[24px] text-gray-800 dark:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">
                  <path stroke="gray" stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M3.933 13.909A4.357 4.357 0 0 1 3 12c0-1 4-6 9-6m7.6 3.8A5.068 5.068 0 0 1 21 12c0 1-3 6-9 6-.314 0-.62-.014-.918-.04M5 19 19 5m-4 7a3 3 0 1 1-6 0 3 3 0 0 1 6 0Z"/>
                </svg>
              </template>
            </div>
          </button>
