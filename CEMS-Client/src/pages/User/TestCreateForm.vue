...
const filterData = () => {
  ...
  if (filters.value.searchRqName.trim()) {
    filtered = filtered.filter((item) =>
      item.rqName
        .toLowerCase()
        .includes(filters.value.searchRqName.toLowerCase())
    );
  }
  ...
  if (filters.value.selectedRequisitionTypeId) {
    ...
    if (!isNaN(reqTypeId)) {
      ...
      const reqType = requisitionTypeStore.requisitionTypes.find(
        (rt) => rt.rqtId === reqTypeId
      );
      if (reqType) {
        filtered = filtered.filter(
          (item) => item.rqRqtName === reqType.rqtName
        );
      }
    }
  }
...
onMounted(async () => {
  ...
  try {
    ...
    await Promise.all([
      expenseReimbursementStore.getAllExpenseReimbursementHistory(
        user.value.usrId
      ),
    ...
    originalData.value = [
      ...expenseReimbursementStore.expenseReimbursementHistory,
    ] as ExpenseHistory[];
  } catch (error) {
    ...
    <div class="w-full border-r-[2px] border-l-[2px] border-t-[2px] mt-12">
      ...
      <table class="table-auto w-full text-center text-black">
        <tbody>
          ...
          <tr
            v-else-if="
              expenseReimbursementStore.expenseReimbursementHistory.length === 0
            "
          >
          ...
          <tr
            ...
            v-for="(
              item, index
            ) in expenseReimbursementStore.expenseReimbursementHistory"
            :key="item.rqId"
