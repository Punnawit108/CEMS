    ...
    try {
      ...
      if (parsedUser && typeof parsedUser === "object") {
        ...
        if (user.value && user.value.usrId) {
          user.value = await notificationStore.loadNotifications(
            user.value.usrId
          );
...
const filteredNotifications = computed(() => {
  ...
  if (filterNotification.value === "read") {
    notifications = notifications?.filter(
      (item: any) => item.ntStatus === "read"
    );
  } else if (filterNotification.value === "unread") {
    notifications = notifications.filter(
      (item: any) => item.ntStatus === "unread"
    );
  }
...
<template>
  <div>
    <nav
      class="flex overflow-hidden items-center whitespace-nowrap mb-2"
    ...
    >
      ...
      >
          ...
          <button
            ...
            :class="[
              ...
              clickAllNotification
                ...
                : 'border-neutral-400 text-neutral-500 text-opacity-80',
            ]"
          >
            <svg
              :style="{ fill: clickAllNotification ? '#0066DD' : '#777777' }"
              width="18"
              height="17"
              viewBox="0 0 18 17"
        ...
        <li>
          <button
            @click="toggleReadedNotification"
            :class="[
              'flex px-4 py-1.5 bg-white rounded-3xl border-2 border-solid',
              clickReadedNotification
                ? 'border-[#12B669] text-[#12B669]'
                : 'border-neutral-400 text-neutral-500 text-opacity-80',
            ]"
          >
            <svg
              :style="{ fill: clickReadedNotification ? '#12B669' : '#777777' }"
              width="18"
              height="17"
              viewBox="0 0 18 17"
              xmlns="http://www.w3.org/2000/svg"
        ...
        <li>
          <button
            @click="toggleNotReadNotification"
            :class="[
              'flex px-4 py-1.5 bg-white rounded-3xl border-2 border-solid',
              clickNotReadNotification
                ? 'border-[#D92C20] text-[#D92C20]'
                : 'border-neutral-400 text-neutral-500 text-opacity-80',
            ]"
          >
            <svg
              :style="{
                fill: clickNotReadNotification ? '#D92C20' : '#777777',
              }"
              width="18"
              height="17"
              viewBox="0 0 18 17"
    ...
    <article class="flex flex-col border border-solid border-[#B6B7BA]">
      ...
      <CardNotification
        ...
        :notificationInfo="paginatedNotifications"
      />
      ...
      <div
        ...
        :key="`empty-${index}`"
        class="flex justify-between h-[85px] pl-4"
      >
        <div
