...
const filteredHistory = computed(() => {
  ...
  // Log การกรองข้อมูล
  console.log('Filtering history with filters:', JSON.stringify(lastSearchedFilters.value));
...
const handleSearch = () => {
  ...
  if (filters.value.startDate) {
    console.log('Start date for filtering (คริสต์ศักราช):', filters.value.startDate);
    console.log('Start date for filtering (พุทธศักราช):', formatToBuddhistYYYYMMDD(filters.value.startDate));
  }
  
  if (filters.value.endDate) {
    console.log('End date for filtering (คริสต์ศักราช):', filters.value.endDate);
    console.log('End date for filtering (พุทธศักราช):', formatToBuddhistYYYYMMDD(filters.value.endDate));
  }
};
...
onMounted(async () => {
  ...
  try {
    ...
    if (user.value) {
      ...
      await expenseReimbursementStore.getAllExpenseReimbursementHistory(user.value.usrId);
      ...
      if (expenseReimbursementHistory.value && expenseReimbursementHistory.value.length > 0) {
        ...
        const secondItem = expenseReimbursementHistory.value.length > 1 ? expenseReimbursementHistory.value[1] : null;
        ...
        if (firstItem.rqWithDrawDate && firstItem.rqWithDrawDate.includes('-')) {
        ...
        <table class="table-auto w-full text-center text-black">
          <tbody>
            ...
            <tr v-else-if="!expenseReimbursementHistory?.length">
              <td colspan="8" class="py-4">ไม่มีข้อมูลประวัติการเบิกค่าใช้จ่าย</td>
            </tr>
            ...
            <tr v-else v-for="(expenseReimbursementItem, index) in paginated"
              ...
              <th class="py-[10px] px-2 w-20 text-center">
                <span @click="toDetails(expenseReimbursementItem.rqId)" class="flex justify-center cursor-pointer hover:text-[#B67D12]">
