...
const { approvalHistory } = storeToRefs(approvalStore);
...
const filteredApprovals = computed(() => {
  if (!approvalHistory.value) return [];

  // Log การกรองข้อมูล
  console.log(
    "Filtering approvals with filters:",
  ...
  return approvalHistory.value.filter((item) => {
    // กรองตามชื่อผู้ใช้
    const matchesSearch =
    ...
    const matchesRequisitionType =
      ...
      (item.rqtName &&
        item.rqtName === lastSearchedFilters.value.requisitionType);
...
const extractedRequisitionTypes = computed(() => {
  ...
  approvalHistory.value.forEach((item) => {
    if (item.rqtName) {
      uniqueTypes.set(item.rqtName, {
        rqtId: item.rqtName,
        rqtName: item.rqtName,
      });
    }
  });
    ...
    >
      ...
      <table class="w-full text-center text-black table-auto">
        <tbody>
          ...
          <tr
            v-else-if="
              !approvalHistory?.length || filteredApprovals.length === 0
            "
            v-for="n in 10"
            :key="n"
            class="h-[50px]"
          ...
          <tr
            ...
            :key="item ? item.rqId : `empty-${index}`"
            :class="
              item
          ...
          >
            <template v-if="item">
              ...
              <th
                class="py-3 px-2 text-start w-44"
                style="
                  max-width: 200px;
                  white-space: nowrap;
                  text-overflow: ellipsis;
              ...
              <th
                ...
                class="py-3 px-2 w-20 cursor-pointer hover:text-[#B67D12]"
              >
