...
interface Props {
  modelValue: string  // ค่าฝ่ายที่เลือก
...
const emit = defineEmits<{
  'update:modelValue': [value: string]  // event เมื่อมีการเปลี่ยนแปลงค่าฝ่ายที่เลือก
}>()

// คำนวณรายชื่อฝ่ายที่ไม่ซ้ำกันจากข้อมูลผู้ใช้
    ...
    <form class="grid">
      ...
      <div class="relative h-[32px] w-full justify-center items-center">
        <select :value="modelValue" @change="$emit('update:modelValue', ($event.target as HTMLSelectElement).value)"
          id="SelectDivision" :disabled="loading" :class="[
