...
watch(
  () => props.searchTerm,
  (newVal) => {
    localFilters.value.searchTerm = newVal;
  }
);
watch(
  () => props.searchRqName,
  (newVal) => {
    localFilters.value.searchRqName = newVal;
  }
);
watch(
  () => props.selectedDepartment,
  (newVal) => {
    localFilters.value.selectedDepartment = newVal;
  }
);
watch(
  () => props.selectedDivision,
  (newVal) => {
    localFilters.value.selectedDivision = newVal;
  }
);
watch(
  () => props.selectedRole,
  (newVal) => {
    localFilters.value.selectedRole = newVal;
  }
);
watch(
  () => props.selectedProjectId,
  (newVal) => {
    localFilters.value.selectedProjectId = newVal;
  }
);
watch(
  () => props.selectedRequisitionTypeId,
  (newVal) => {
    localFilters.value.selectedRequisitionTypeId = newVal;
  }
);
...
const handleSearch = () => {
  ...
  emit(
    ...
    localFilters.value.selectedProjectId?.toString() || ""
  );
  emit(
    "update:selectedRequisitionTypeId",
    localFilters.value.selectedRequisitionTypeId?.toString() || ""
  );
...
<template>
  <div
    class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 mb-12 w-full [&>*:last-child]:xl:col-start-5"
  >
    <!-- Search Filter -->
    <div v-if="showSearchFilter" class="h-[32px] min-w-[200px] flex-1">
      <form class="grid" @submit.prevent>
        <label for="SearchBar" class="py-0.5 text-[14px] text-black text-start"
          >ค้นหา</label
        >
      ...
      <form class="grid" @submit.prevent>
        ...
        <div class="relative h-[32px] w-full justify-center items-center">
          ...
          <input
            type="text"
            id="SearchRqName"
            :value="localFilters.searchRqName"
            @input="updateSearchRqName"
            class="appearance-none text-sm flex justify-between w-full h-[32px] bg-white rounded-md border border-black border-solid focus:outline-none focus:border-blue-500 pl-9"
    ...
    <div v-if="showDepartmentFilter" class="h-[32px] min-w-[200px] flex-1">
      <form class="grid">
        ...
        <div class="relative h-[32px] w-full justify-center items-center">
          <select
            :value="localFilters.selectedDepartment"
    ...
    <div v-if="showDivisionFilter" class="h-[32px] min-w-[200px] flex-1">
      <form class="grid">
        ...
        <div class="relative h-[32px] w-full justify-center items-center">
          <select
            :value="localFilters.selectedDivision"
    ...
    <div v-if="showRoleFilter" class="h-[32px] min-w-[200px] flex-1">
      <form class="grid">
        <label for="SelectRole" class="py-0.5 text-[14px] text-black text-start"
          >บทบาท</label
        >
        <div class="relative h-[32px] w-full justify-center items-center">
          <select
            :value="localFilters.selectedRole"
            @change="updateRole"
            id="SelectRole"
            :disabled="loading"
            class="custom-select appearance-none text-sm flex justify-between w-full h-[32px] bg-white rounded-md border border-black border-solid focus:outline-none focus:border-blue-500 pl-4 pr-8"
    ...
    <div v-if="showProjectFilter" class="h-[32px] min-w-[200px] flex-1">
      <form class="grid">
        ...
        <div class="relative h-[32px] w-full justify-center items-center">
          <select
            :value="localFilters.selectedProjectId"
    ...
    <div v-if="showRequisitionTypeFilter" class="h-[32px] min-w-[200px] flex-1">
      <form class="grid">
        <label
          for="SelectRequisitionType"
          class="py-0.5 text-[14px] text-black text-start"
          >ประเภทค่าใช้จ่าย</label
        >
        <div class="relative h-[32px] w-full justify-center items-center">
          <select
            :value="localFilters.selectedRequisitionTypeId"
            @change="updateRequisitionType"
          ...
          >
            <option value="">ทั้งหมด</option>
            <option
              v-for="type in requisitionTypes"
              :key="type.rqtId"
              :value="type.rqtId"
            >
              {{ type.rqtName }}
    ...
    <template v-if="showDateFilter">
      ...
      <div class="h-[32px] min-w-[200px] flex-1">
        <form class="grid">
          <label class="py-0.5 text-[14px] text-black text-start"
