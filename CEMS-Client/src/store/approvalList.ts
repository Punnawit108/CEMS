/*
 * ชื่อไฟล์: approvalList.ts
 * คำอธิบาย: ไฟล์ store API ของรายการคำขออนุมัติ และประวัติคำขออนุมัติ
...
export const useApprovalStore = defineStore("approvalList", {
  ...
  actions: {
    ...
    async getApprovalList(id: string) {
      try {
        const result = await axios.get(
          `${import.meta.env.VITE_BASE_URL}/api/approvalList/list/${id}`
        );
    ...
    async getApprovalHistory(id: string) {
      try {
        const result = await axios.get(
          `${import.meta.env.VITE_BASE_URL}/api/approvalList/history/${id}`
        );
