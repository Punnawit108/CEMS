...
export const useRequisitionStore = defineStore("dropdown", {
  ...
  getters: {
    ...
    filteredVehicleType: (state) => {
      return state.vehicleType.filter(
        (vehicle) =>
  ...
  actions: {
    ...
    async getAllProject() {
      try {
        const result = await axios.get(
          `${import.meta.env.VITE_BASE_URL}/api/dataType/project`
        );
    ...
    async getAllRequisitionType() {
      try {
        ...
        this.requisitionType = result.data.filter(
          (item: ExpenseManage) => item.rqtVisible === 1
        );
    ...
    async getAllVehicleType() {
      try {
        const result = await axios.get(
          `${import.meta.env.VITE_BASE_URL}/api/dataType/vehicle`
        );
    ...
    async getUserEmail(usrId: string) {
      try {
        const result = await axios.get(
          `${import.meta.env.VITE_BASE_URL}/api/user/email/${usrId}`
        );
    ...
    async getRqCode() {
      try {
        const result = await axios.get(
          `${import.meta.env.VITE_BASE_URL}/api/expense/next-rq-code`
        );
    ...
    async deleteFile(fId: number) {
      try {
        const result = await axios.delete(
          `${import.meta.env.VITE_BASE_URL}/api/expense/file/${fId}`
        );
        return result.data;
      } catch (error) {
        if (axios.isAxiosError(error)) {
