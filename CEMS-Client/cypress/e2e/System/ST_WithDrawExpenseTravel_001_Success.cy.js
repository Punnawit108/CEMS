describe("ST_WithDrawExpenseTravel_001_Success", () => {
  beforeEach(() => {
    //1
  ...
  it("ตรวจสอบการทำงานของของระบบ เมื่อสร้างใบเบิกเป็นค่าเดินทาง รถสาธารณะ", () => {
    ...
    for (let i = 2; i <= 7; i++) {
      // สร้าง XPath พร้อมกับเปลี่ยนตำแหน่งของ <th>
      let xpath = `//*[@id="app"]/div/div/div/div/div[2]/div/div[2]/table[11]/tbody/tr/th[${i}]`;

      // สำหรับตำแหน่งที่ 7 มีโครงสร้าง XPath ต่างออกไป
      if (i === 7) {
      ...
      cy.xpath(xpath)
        .invoke("text") // ดึงข้อความจาก element
        .then((text) => {
          const trimmedText = text.trim(); // ตัดช่องว่างข้างเคียง
          expect(trimmedText).to.eq(expectedListData[i - 2]); // ตรวจสอบว่าข้อความตรงกับข้อมูลใน array
