describe("ST_WithDrawExpenseTravel_002_Success", () => {
  beforeEach(() => {
    //1
    cy.login("65160341", "admin");
  });

  it("ตรวจสอบการทำงานของของระบบ เมื่อสร้างใบเบิกเป็นค่าเดินทาง รถส่วนตัว", () => {
    //2
    ...
    cy.get("#travelType").select("private");
    cy.wait(1000);
    cy.get("#vehicle").select("รถยนต์");
